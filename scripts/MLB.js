...
async function createExpandedWidget() {
    ...
    if (isPlaying) {
        ...
        if (game.teams.away.probablePitcher === undefined) {
            const playerNameText = nameCountStack.addText("TBD");
            playerNameText.font = Font.regularSystemFont(12);
            playerNameText.textColor = Color.white();
        } else {
            ...
            nameCountStack.addSpacer(4);
            const winnerStats =
                game.teams.away.probablePitcher.stats.filter(stat => stat.type.displayName === 'statsSingleSeason' &&
                    stat.group.displayName === 'pitching')[0].stats;
        ...
        if (game.teams.home.probablePitcher === undefined) {
            const pitcherNameText = namePitchesStack.addText("TBD");
            pitcherNameText.font = Font.regularSystemFont(12);
            pitcherNameText.textColor = Color.white()
        } else {
            ...
            // pitcherNameText.minimumScaleFactor = 0.9;
            namePitchesStack.addSpacer(4);
            const loserStats = game.teams.home.probablePitcher.stats.filter(
                    stat => stat.type.displayName === 'statsSingleSeason' && stat.group.displayName === 'pitching')[0]
                .stats;
